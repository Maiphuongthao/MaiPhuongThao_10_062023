...
class ContributorViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return response.Response('Cet utilisateur existe deja.', status=status.HTTP_400_BAD_REQUEST)
        except Contributor.DoesNotExist:
            if user == project.author_user_id:
                return response.Response("l'author du project ne peut pas être contributor.", status=status.HTTP_400_BAD_REQUEST)
            else:
                serializer = ContributorSerializer(data = request.data)
                if serializer.is_valid():
                    serializer.save()
                    return response.Response({"status": "success", "data": serializer.data}, status=status.HTTP_200_OK) 
                else:  
                    return response.Response({"status": "error", "data": serializer.errors}, status=status.HTTP_400_BAD_REQUEST)  
